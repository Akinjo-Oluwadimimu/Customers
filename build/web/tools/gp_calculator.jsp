...
<input type="hidden" id="status" name="status" value="<%= request.getAttribute("done")%>">
      ...
      <ul class="nav col-12 col-md-auto mb-2 justify-content-center mb-md-0">
        ...
        <li class="dropdown">
                  ...
                  <ul class="dropdown-menu">
                    <li><a class="dropdown-item" href="${pageContext.request.contextPath}/student/transcript.jsp#eng">English Styled</a></li>
                    <li><a class="dropdown-item" href="${pageContext.request.contextPath}/student/transcript.jsp#cn">Chinese Styled</a></li>
                    <li><a class="dropdown-item" href="${pageContext.request.contextPath}/student/transcript.jsp#wes">Western Styled</a></li>
                  </ul>
        </li>
        ...
        <li class="dropdown">
                  ...
                  <ul class="dropdown-menu">
                    <li><a class="dropdown-item" href="#">GP Calculator</a></li>
...
<nav class="small" id="toc">
 ...
 <div class="bd-cheatsheet container-fluid bg-white center">
    <article class="pt-3 pt-xl-5 pb-2 pb-xl-3">
      <p style="text-align: center" class="display-5">Grade Point Average Calculator</p>
      <div id='containers' class="container">
      <div id="POItablediv">
          <form action="testing" method="get" name="myForm">
          <input type="hidden" name="action" value="testing">
        <table class="table table-sm table-bordered" id="POITable">
          <thead>
          <tr>
            <th scope="col"></th>
          ...
          <tbody>
              <c:choose>
                        <c:when test="${done =='done' || done =='error'}">
                <c:forEach var="item" items="${calculator}">
                            <tr id="calculator">
                      <th scope="row">
                          ${item.id}
                      </th>
                      <td>
                          <input type="text" id="course<c:out value='${item.id}'/>" name="course" value="${item.course}"/>
                      </td>
                      <td>
                          <input type="number" step="0.5" id="unit<c:out value='${item.id}'/>" name="unit" value="${item.unit}"/>
                      </td>
                      <td>
                          <input type="text" id="grade<c:out value='${item.id}'/>" name="grade" value="${item.grade}"  onblur="checkGrade()"/>
                      </td>
                      <td><button class="btn btn-close" id="delPOIbutton" onclick="deleteRow(this)"></button></td>
                    </tr>
                </c:forEach>
                </c:when><c:otherwise>
          <tr>
            ...
            <td><input type="number" step="0.5" id="unit" name="unit"/></td>
            <td><input type="text" id="grade" name="grade" onblur="checkGrade()"/></td>
            <td><button class="btn btn-close" id="delPOIbutton" onclick="deleteRow(this)"></button></td>
          </tr>
          ...
          <div class="col-12 d-flex justify-content-center">
            <button class="btn btn-light border" type="button" id="addmorePOIbutton" onclick="insRow()">Add Row</button>
          </div>
          <button style="float: left" type="submit" class="btn btn-light border">Calculate GPA</button>
          <button style="float: right" type="button" class="btn btn-light border">Clear grades</button>
          </form>
          </div>
      <div style="clear: both; padding-top: 15px" id="alert"></div>
      <c:if test="${done =='done'}">
      <div style="margin-top: 15px" class="container" id="containers">
      <p style="text-align: center" class="lead">Chinese Grading System</p>
      <table class="table table-sm table-bordered">
          ...
          <tbody>
          <c:forEach var="listitems" items="${calculator}">
          <c:if test="${listitems.unit != '' && listitems.grade != ''}">
          <tr>
            <th scope="row">${listitems.id}</th>
            <td>${listitems.course}</td>
            <td>${listitems.unit}</td>
            <td>${listitems.grade}</td>
            <td>${listitems.cngp}</td>
          </tr>
          <tr>
        </c:if>
    </c:forEach>
        ...
        <p>Cumulative GPA: ${cngpa}</p>
          </div>
          
          <div style="margin-top: 15px" class="container" id="containers">
      <p style="text-align: center" class="lead">US Grading System</p>
      <table class="table table-sm table-bordered">
          <thead>
          <tr>
            <th scope="col"></th>
          ...
          <tr>
            ...
            <td>${listitems.unit}</td>
            <td>${listitems.grade}</td>
            <td>${listitems.usgrade}</td>
            <td>${listitems.usgp}</td>
          </tr>
          <tr>
        </c:if>
    </c:forEach>
...
var status = $('#status').val();
    if(status==="error"){
        alert("The number of credits for one or more courses is missing or incorrect. If all your grades have the same weight, you can enter 1 for each class.");
    }
};
...
function addActionRow(row) {
  ...
  var inp3 = row.cells[3].getElementsByTagName('input')[0];
  if(inp3){
      inp3.addEventListener("blur", GradeRespondBlur);
  }
  
   function GradeRespondBlur() {
    var grade = inp3.value;
    var regexnumber=/^[+]?((\d+(\.\d*)?)|(\.\d+))$/;
    var regexletter=/^[A-DF]?$/;
    if (!grade.match(regexnumber))
    {
        if (!grade.match(regexletter)){
