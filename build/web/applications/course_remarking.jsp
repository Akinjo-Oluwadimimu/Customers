...
<html lang="en">
    <head>
        ...
        <title>Result Rechecking Application</title>
...
<div class="bd-cheatsheet container-fluid bg-white">
  ...
  <div class="container" id="containers">
      <p style="text-align: center" class="display-5">Result Rechecking Application</p>
      <p class="lead" style="text-align: center">Fill in the form below to apply for result rechecking. </p>
      <form name="myForm" id="file" action="courseRemarking" method="post" enctype="multipart/form-data">
          <input type="hidden" name="action" value="courseRemarking">
          <div class="col-md-12">
            <label for="select_page" class="form-label">Course Name</label>
            <select id="select_page" required name="courseName" class="operator"> 
          ...
          <div class="col-md-12  mb-3">
            ...
            <input type="text" name="expectedResult" id="expectedResult" class="form-control"  onblur="checkInp()" required>
          </div>
          <div class="col-md-12 input-group  mb-3">
            <span class="input-group-text">Application Reason</span>
            <textarea id="reason" class="form-control" aria-label="With textarea" placeholder="Application reason (maximum of 1000 characters)" maxlength="1000" required name="applicationReason"></textarea>
          </div>
          <div class="col-md-12 input-group  mb-3">
            <span class="input-group-text">Supporting Evidence</span>
            <textarea id="evidence" class="form-control" aria-label="With textarea" placeholder="Supporting evidence (maximum of 1000 characters)" maxlength="1000" required name="supportingEvidence"></textarea>
          </div>
          <div class="mb-3">
            <label class="form-label" for="image_upload">Upload Evidence</label>
            <input type="file" id="image_upload" name="file" multiple class="form-control" accept="image/jpg, image/jpeg, image/png" aria-describedby="uploadHelp">
            <div id="uploadHelp" class="form-text">Please Upload any Supporting Evidence you have (maximum of three images).</div>
          </div>
...
<div class="modal fade" id="formSubmit" data-bs-backdrop="static" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
    <div class="modal-content">
      ...
      <div class="modal-body">
          <p class="lead"><b>Are you sure you want to submit the following?</b></p>
          <div id="appendInfo"></div>
          <p id="status"></p> <div class="d-flex justify-content-center"><img id="output0" width="350px"></div>
              <br>
              <div class="d-flex justify-content-center"><img id="output1" width="350px"></div>
              <br>
              <div class="d-flex justify-content-center"><img id="output2" width="350px"></div>
      </div>
      <div class="modal-footer">
        <button type="button" id="cancel" class="btn btn-secondary" data-bs-dismiss="modal">No</button>
        <button type="button" id="upload" class="btn btn-primary">Yes</button>
      </div>
    </div>
  </div>
</div>  

  <div class="modal fade" id="submitDone" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
...
const alertPlaceholder = document.getElementById('appendInfo')    
const placealert = (course, expectedResult, reason, evidence) => {
  const wrapper = document.createElement('div')
  wrapper.innerHTML = [
    '<p><b>Course Name:</b> <br>'+course+'</p><p><b>Expected Result:</b> <br>'+expectedResult+'</p><p><b>Application Reason:</b> <br>'+reason+
            '<p><b>Supporting Evidence:</b> <br>'+evidence+'<p><b>Images:</b><br></p>'
  ].join('')
...
const course = document.getElementById("select_page");
...
if (submitbutton) {
  submitbutton.addEventListener('click', () => {
      if(course.value==''||expectedResult.value==''||reason.value==''||evidence.value==''){
          alert('please fill in all required fields')
      }else{
   placealert(course.value, expectedResult.value, reason.value, evidence.value);
    OpenBootstrapPopup();}
  })
}
      ...
      if (window.FileList && window.File && window.FileReader) {
        document.getElementById('image_upload').addEventListener('change', event => {
          ...
          reader.addEventListener('load', event => {
            output0.src = event.target.result;
...
if (uploadbutton) {
  ...
  function checkInp()
{
    var x=document.forms["myForm"]["expectedResult"].value;
    const expectedResult = document.getElementById('expectedResult');
    var regexnumber=/^[+-]?((\d+(\.\d*)?)|(\.\d+))$/;
    var regexletter=/^[A-DF]?$/;
    if (!x.match(regexnumber))
    {
        if (!x.match(regexletter)){
...
$(function() {
  ...
  $file_upload.on('change', function () {
    ...
    if (number_of_images > max_file_number || number_of_images < min_file_number) {
      alert(`You can upload a minimum of 1 and a maximum of 3 files.`);
      $(this).val('');
      $button.prop('disabled', 'disabled');
    } else {
